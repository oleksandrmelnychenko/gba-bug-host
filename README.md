...
2. Worker атомарно забирає наступний job і обробляє рівно одну coding-задачу.
...
`CODEX_REASONING_EFFORT` задається явно (типово `high`), щоб worker не успадковував випадковий host-wide `max`/`ultra` і не витрачав увесь job timeout до першої перевірки репозиторію. За потреби його можна підняти для окремого контрольованого запуску; незнайоме значення зупиняє worker fail-closed.
